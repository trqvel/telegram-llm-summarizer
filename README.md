...
Анализ тональности сообщений реализован в файле src/llm/llama/sentiment.R, который:
...
Суммаризация сообщений реализована в файле src/llm/llama/summarize.R, который:
...
Для суммаризации используются современные модели NLP, такие как GPT (через API Llama) или локально развернутые модели через библиотеку huggingfaceR. Система формирует запрос к модели, включающий контекст группы, временной период и сами сообщения, а затем получает сгенерированное резюме.
...
Генерация RSS-потоков реализована в файле src/llm/llama/gen_rss.R, который:
